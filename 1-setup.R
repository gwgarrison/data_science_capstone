...
dm <- removeSparseTerms(bdm, 0.99995)
tm <- removeSparseTerms(tdm, 0.99995)
um <- removeSparseTerms(udm, 0.99995)
...
quad.freq <- tbl_df(data.frame(q.freq[,2],q.freq[,1]))
names(bi.freq) <- c("bigram","count")
names(tri.freq) <- c("trigram","count")
names(uni.freq) <- c("unigram","count")
names(quad.freq) <- c("quadgram","count")

save(bi.freq,tri.freq,uni.freq,quad.freq,freq,wf,file = "freq.rda")
